...
Website Link: http://andrewnorcross.com/plugins/mojoreferral-link-creator/
...
Creates a mojoreferral generated shortlink on demand or when saving posts.
...
= 2.1.1 - 02/25/2016 =
* added `mojoreferral_run_cron_jobs` filter to allow users to disable the cron job

= 2.1.0 - 08/06/2015 =
* added `mojoreferral_after_url_save` action to post transitioning from future to publish (scheduled posts)

= 2.0.9 - 08/03/2015 =
* added `mojoreferral_after_url_save` action to fire when the custom URL is generated and stored

= 2.0.8 - 06/19/2015 =
* added ability to set custom keyword prior to save
* added `mojoreferral_user_cap` filter for modifying default user capability
* added `mojoreferral_settings_cap` filter for modifying settings page capability
* added `mojoreferral_keyword_filter` filter for changing default keyword sanitation
...
* added `mojoreferral_post_status` filter to modify these (although not sure why you'd want to)
...
* added filter `mojoreferral_strip_urls` to allow trailing slash removal (default is false)
...
* added `mojoreferral_display_box` template tag to display front end box
* added `get_mojoreferral_shortlink` to return or echo the shortlink
* added `wp_ozh_mojoreferral_raw_url` to match previous plugin from Ozh
...
* added check for possible 404 return on mojoreferral server
* a whole lot more
...
= 1.07 - 12/24/2012 =
* better sanitizing of personal mojoreferral URL
* code cleanup

= 1.06 - 12/18/2012 =
* included template tag for theme use. (See FAQs for usage)
* added a 'delete' button for single links (See FAQs for details)
* The mojoreferral metabox will not appear until a post has been published.
